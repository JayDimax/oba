...
function updateAvailableGamesForToday() {
  ...
  let disableDrawButtonsDueToCutoff = false; // Flag for the strict cutoff period
  let isScheduleForTomorrow = false; // Flag indicating if we show tomorrow's schedule

  // ✅ Between 8:50 PM and 9:00 PM -> disable ALL betting (cutoff period)
  if ((currentHour === 20 && currentMinute >= 50) || (currentHour === 21 && currentMinute <= 0)) {
    disableDrawButtonsDueToCutoff = true;
  }

  // ✅ After 9:01 PM -> switch to tomorrow's schedule
  const isAfter901PM = currentHour > 21 || (currentHour === 21 && currentMinute >= 1);
  if (isAfter901PM) {
    targetDate.setDate(now.getDate() + 1);
    isScheduleForTomorrow = true; // Mark that the schedule displayed is for tomorrow
  }
  ...
  // --- Update Draw Time Buttons ---
  drawTimeInputs.forEach(drawBtn => {
    // Reset classes for each button in case state changes
    drawBtn.classList.remove('opacity-50', 'cursor-not-allowed', 'pointer-events-none');
    drawBtn.disabled = false; // Assume enabled unless a rule disables it

    // 1. Strictest priority: Cutoff period (8:50 PM - 9:00 PM)
    if (disableDrawButtonsDueToCutoff) {
      drawBtn.disabled = true;
      drawBtn.classList.add('opacity-50', 'cursor-not-allowed', 'pointer-events-none');
      return; // Skip further checks if in cutoff
    }

    // 2. Next priority: If schedule is for tomorrow, enable all draw times for selection
    if (isScheduleForTomorrow) {
        // All draw times are valid for tomorrow's betting
        drawBtn.disabled = false;
        return;
    }

    // 3. Default logic for today's schedule: Disable past draw times
    // Parse the draw hour from the button's text/content.
    const drawTimeLabel = drawBtn.textContent.trim();
    const drawHour = parseDrawHour(drawTimeLabel);

    if (drawHour === -1) {
        console.warn(`Could not parse draw time from button:`, drawBtn);
        drawBtn.disabled = false; // Enable if parsing fails, to avoid blocking bets
        return;
    }

    // Compare the parsed draw hour with the current time
    // Disable if the draw hour is less than current hour,
    // OR if it's the same hour but the draw minute (assumed 00) is less than/equal to current minute.
    // This correctly handles the 10-minute pre-draw cutoff for the current day's draws.
    if (drawHour < currentHour || (drawHour === currentHour && currentMinute >= 0)) {
        drawBtn.disabled = true;
        drawBtn.classList.add('opacity-50', 'cursor-not-allowed', 'pointer-events-none');
    } else {
        drawBtn.disabled = false;
