...
public function admin(Request $request)
{
    ...
    // =========[ FINANCIAL SUMMARY ]=========
    $totalGross = Bet::whereDate('created_at', $today)->sum('amount');
// 

    $totalWinningBetAmount = Bet::whereDate('created_at', $today)
        ->where('is_winner', 1)
        ->sum('amount');
    ...
    $netAfterCommission = $totalGross - $commission;
    ...
    $totalCommission = $bets->sum(fn($bet) => ($bet->commission ?? 0) + ($bet->commission_bonus ?? 0));
    ...
    return view('admin.dashboard', compact('totalGross','commission','netAfterCommission','totalWinnings','expectedRemittance','actualRemittance','unremittedBalance',    'deficit',
        'netSales','totalCommission','totalAgents','activeAgents','blockedAgents','agentsWithBalance','nextDraw','latestResult','agents','bets','from','to','draw',
        'agentCodes','agentName','topCombinations'));
    }
