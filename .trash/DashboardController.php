...
class DashboardController extends Controller
{
    ...
    public function exportBets(Request $request): StreamedResponse
    {
        $from = $request->input('from_date');
        $to = $request->input('to_date');
        $drawTime = $request->input('draw_time');
        $agentName = $request->input('agent_name');

        $drawMap = ['2PM' => '14', '5PM' => '17', '9PM' => '21'];
        $drawTimeValue = $drawTime !== 'ALL' && isset($drawMap[$drawTime])
            ? $drawMap[$drawTime]
            : null;

        $bets = Bet::whereBetween('game_date', [$from, $to])
            ->when($drawTimeValue, fn($q) => $q->where('game_draw', $drawTimeValue))
            ->when($agentName, fn($q) => $q->whereHas('betAgent', fn($subQ) =>
                $subQ->where('name', 'like', "%{$agentName}%")
            ))
        ...
        $callback = function () use ($bets) {
            ...
            fputcsv($file, ['Agent ID', 'Agent Name', 'Stub ID', 'Bet Number', 'Amount', 'Game Date']);
