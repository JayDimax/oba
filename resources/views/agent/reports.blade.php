    ...
    <div class="max-w-5xl mx-auto p-4" x-data="{ tab: '{{ $draw ?? 'all' }}' }">
      ...
      <div class="bg-gray-50 dark:bg-gray-800 rounded-lg shadow px-4 py-3 min-h-32 flex flex-col justify-between">
        ...
        <div class="space-y-1 text-sm">
          ...
          <div class="flex justify-between"><span>Winning Amount:</span><span>{{ number_format($summary['payouts'], 2) }}</span></div>
          <div class="flex justify-between"><span>Commission:</span><span>{{ number_format($summary['commission_base'], 2) }}</span></div>
          ...
          @if ($summary['deficit'] > 0)
              <div class="text-red-600 font-bold mt-2">
                  Tapada: ₱ {{ number_format($summary['tapada'], 2) }}
              </div>
          @endif
